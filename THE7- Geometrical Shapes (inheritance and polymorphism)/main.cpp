...
shape* getShape(){
	...
	// getting the variables depending to the option
	float width, length, height;
	string name;
	if(option == "1"){
		
		cout << "You choose rectangle. Give it's width, length and name: " << endl;
		cin >> width;
		cin >> length;
		cin >> name;
		// forming the rectangle object pointer and returning it
		rectangle* myrec = new rectangle(width, length, name);
		result = myrec;

	}
	else{ // if option == 2

		cout << "You choose box. Give it's width, length, height and name: " << endl;
		cin >> width;
		cin >> length;
		cin >> height;
		cin >> name;
		// forming the box object pointer and returning it
		box* mybox = new box(width, length, height, name);
		result = mybox;
...
int main() // main function is taken from Artrim Kjamilji
{
